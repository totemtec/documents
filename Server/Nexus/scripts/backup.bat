...
:backup
    @echo 备份文件打包中，请耐心等待
	ssh root@%local_nexus% "docker stop --time=120 nexus && cd /opt/nexus/data && rm -rf archive.tar.gz && tar -czf archive.tar.gz BackUp keystores/node blobs && docker start nexus && rm -rf BackUp/*.bak"
	@echo 备份文件打包完成。
